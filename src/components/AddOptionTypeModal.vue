<template>
  <v-dialog max-width="600">
    <v-card>
      ...
      <v-col>
        <div>
          <div v-for="(option, index) in options" :key="index" style="display: flex; align-items: center; margin-bottom: 8px;">
            <v-text-field
...
const options = ref([""]); // 初始包含一個空字串的輸入框
...
const addOption = () => {
  options.value.push(""); // 新增一個空的選項輸入框
};

const removeOption = (index) => {
  options.value.splice(index, 1); // 刪除指定索引的輸入框
};

const validateOptions = () => {
  // 當輸入框的空白數超過兩個時，自動刪除多餘的空白框
  const emptyCount = options.value.filter((option) => option.trim() === "").length;
  if (emptyCount > 2) {
    options.value = options.value.filter((option, index) => option.trim() !== "" || index < 2);
  }
};
